...
export function useFetchFromBackendGameActions(): ReturnGameFunctions {
    ...
    const toGames = async (convexGames: ConvexGame[]): Promise<Game[]> => {
        return Promise.all(
            convexGames.map(async(g) => await toGame(g))
        )
    }

    const toGame = async (convexGame: ConvexGame): Promise<Game> => {
        const gameId = convexGame.id
        const convexCells: ConvexCell[] = await convex.query(api.controllers.cell.queries.getForGame, {gameId: gameId})
        const convexTiles: ConvexTile[] = await convex.query(api.controllers.tile.queries.getForGame, {gameId: gameId})
        const convexPlayers: ConvexPlayer[] = await convex.query(api.controllers.player.queries.getForGame, {gameId: gameId})

        const getTiles = (convexTiles: ConvexTile[]): Tile[] => {
            return convexTiles.map((convexTile: ConvexTile) => tileSchema.parse(convexTile));
        }

        const getPlayers = (convexPlayers: ConvexPlayer[], convexTiles: ConvexTile[]): Player[] => {
            return convexPlayers.map((convexPlayer: ConvexPlayer) => playerSchema.parse({
                ...convexPlayer,
                tiles: convexTiles.filter((convexTile: ConvexTile) => convexTile._playerId === convexPlayer.id)
            }))
        }

        const getCells = (convexCells: ConvexCell[], convexTiles: ConvexTile[]): Cell[] => {
            return convexCells.map((convexCell: ConvexCell): Cell => {
                const cellTiles: ConvexTile[] = convexTiles.filter(convexTile => convexTile._cellId === convexCell.id)
                const cellTile: Tile | null = cellTiles.length > 0 ? tileSchema.parse(cellTiles[0]) : null
