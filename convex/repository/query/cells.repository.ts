...
export interface CellsQueryRepositoryInterface
  extends QueryRepositoryInterface<"cells"> {
  findAllForGame: (gameId: Id<"games">) => Promise<Doc<"cells">[]>;
  findByTile: (tileID: Id<"tiles">) => Promise<Doc<"cells"> | null>;
  findByRow: (
    gameId: Id<"games">,
    row: number,
    min: number,
...
export class CellsQueryRepository implements CellsQueryRepositoryInterface {
  ...
  async findAllForGame(gameId: Id<"games">): Promise<Doc<"cells">[]> {
    return this.db
      .query("cells")
  ...
  async findByTile(tileId: Id<"tiles">): Promise<Doc<"cells"> | null> {
    return this.db
      .query("cells")
  ...
  async findCellInCrossFromCell(
    ...
    const right: Doc<"cells">[] = await this.findByRow(
      cell.gameId,
      cell.row,
      cell.column + 1,
      Math.min(GAME_SIZE, cell.column + 3),
    );
    const up: Doc<"cells">[] = await this.findByColumn(
      cell.gameId,
      cell.column,
      Math.max(0, cell.row - 2),
      cell.row,
    );
    const down: Doc<"cells">[] = await this.findByColumn(
      cell.gameId,
      cell.column,
      cell.row + 1,
