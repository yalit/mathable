...
export class DisplaceTileUseCase {
  ...
  private readonly moveScoreService: ScoreServiceInterface;
  ...
  constructor(ctx: AppMutationCtx) {
    ...
    this.moveScoreService = this.ctx.container.get("ScoreServiceInterface");
  ...
  async execute(
    ...
    await this.cellComputationService.computeAllowedValuesForUpdatedCell(
      fromCell,
    );
    await this.cellComputationService.computeAllowedValuesForUpdatedCell(
      toCell,
