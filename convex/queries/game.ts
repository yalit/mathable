...
export const get = withRepositoryQuery({
    ...
    handler: async (ctx, args): Promise<Game | null> => {
        ...
        const gameCells = await CellsQueryRepository.instance.findForGame(game._id)
        ...
        const gamePlayers = await PlayersQueryRepository.instance.findByGame(game._id);
...
export const getNonFinishedForSession = withSessionQuery({
    ...
    handler: async (ctx, _): Promise<Game[]> => {
        const convexGames = await GamesQueryRepository.instance.findNonFinishedGamesForSessionId(ctx.sessionId)
